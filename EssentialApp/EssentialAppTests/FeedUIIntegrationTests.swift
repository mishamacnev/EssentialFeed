...
//
//  Created by Mikhail Macnev on 25.04.2021.
//
...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expected another loading request ones user initiates a load")
        
        sut.simulateUserInitiatedFeedReaload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a third loading request ones user initiates another load")
    }
    ...
    func test_loadingFeedIndicator_isVisibleWhileLoadingFeed() {
        ...
        sut.loadViewIfNeeded()
        XCTAssertTrue(sut.isShowingLoadingIndicator, "Expected loading indicator ones view is loaded")
        
        loader.completeFeedLoading(at: 0)
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator ones loading conmpletes successfully")
        
        sut.simulateUserInitiatedFeedReaload()
        XCTAssertTrue(sut.isShowingLoadingIndicator, "Expected loading indicator ones user initiates a reload")
        
        loader.completeFeedLoadingWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator ones user initiated loading completes with error")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator state change for second view ones first image loading completes successfully")
        
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view once second image loading completes with error")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view ones second image loading completes with error")
    }
    ...
    func test_feedImageView_doesNotRenderLoadedImageWhenNotVisibleAnymore() {
        ...
        let view = sut.simulateFeedImageViewNotVisibleAnymore(at: 0)
    ...
    private func assertThat(_ sut: FeedViewController, isRendering feed: [FeedImage], file: StaticString = #file, line: UInt = #line) {
        guard sut.numberOfRenderedFeedImagesViews() == feed.count else {
            return XCTFail("Expected \(feed.count) images, got \(sut.numberOfRenderedFeedImagesViews()) instead", file: file, line: line)
        }
