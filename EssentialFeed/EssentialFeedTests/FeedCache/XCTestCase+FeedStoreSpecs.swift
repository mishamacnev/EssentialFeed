    ...
    func expect(_ sut: FeedStore, toRetrieve expectedResult: FeedStore.RetrievalResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retrieve { retrievedResult in
            ...
            case (.success(.none), .success(.none)),
                 (.failure, .failure):
                break
            case let (.success(.some(expectedCache)), .success(.some(retriviedCache))):
                XCTAssertEqual(expectedCache.feed, retriviedCache.feed, file: file, line: line)
                XCTAssertEqual(expectedCache.timestamp, retriviedCache.timestamp, file: file, line: line)
            default:
